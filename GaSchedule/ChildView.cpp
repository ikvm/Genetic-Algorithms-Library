...
CChildView::CChildView() : _running(false)
{
	ScheduleTest::GetInstance().GetObserver().SetWindow( this );
}
...
void CChildView::SetNewState(GaAlgorithmState state)
{
	_running = state == GAS_RUNNING;
}
...
void CChildView::OnPaint() 
{
	...
	if( !_schedule.IsNULL() )
	{
		Schedule* schedule = dynamic_cast<Schedule*>( _schedule.GetRawPtr() );
		CString fit;
		fit.Format( "Fitness: %f, Generation: %d", _schedule->GetFitness(),
			ScheduleTest::GetInstance().GetAlgorithm()->GetAlgorithmStatistics().GetCurrentGeneration() );

		dc.SelectObject( &tableHeadersFont );

		dc.TextOutA( sx + 10, sy + 10, fit );

		const hash_map<CourseClass*, int>& classes = schedule->GetClasses();
		int ci = 0;
		for( hash_map<CourseClass*, int>::const_iterator it= classes.begin(); it != classes.end(); ++it, ci += 5 )
		{
			...
			dc.SetTextColor( schedule->GetCriteria()[ ci + 0 ] ? RGB( 49, 147, 120 ) : RGB( 206, 0, 0 ) );
			dc.TextOut( rect.left, rect.bottom - 10, "R", 1 );
			dc.SetTextColor( schedule->GetCriteria()[ ci + 1 ] ? RGB( 49, 147, 120 ) : RGB( 206, 0, 0 ) );
			dc.TextOut( rect.left + 10, rect.bottom - 10, "S", 1 );
			dc.SetTextColor( schedule->GetCriteria()[ ci + 2 ] ? RGB( 49, 147, 120 ) : RGB( 206, 0, 0 ) );
			dc.TextOut( rect.left + 20, rect.bottom - 10, "L", 1 );
			dc.SetTextColor( schedule->GetCriteria()[ ci + 3 ] ? RGB( 49, 147, 120 ) : RGB( 206, 0, 0 ) );
			dc.TextOut( rect.left + 30, rect.bottom - 10, "P", 1 );
			dc.SetTextColor( schedule->GetCriteria()[ ci + 4 ] ? RGB( 49, 147, 120 ) : RGB( 206, 0, 0 ) );
		...
		int i = 0;
		for( vector<list<CourseClass*>>::const_iterator it = schedule->GetSlots().begin();
			it != schedule->GetSlots().end(); ++it, ++i )
		{
			if( ( *it ).size() > 1 )
			{
...
{
	ScheduleTest::GetInstance().GetAlgorithm()->StartSolving( false );
	return 0;
}
...
{
	ScheduleTest::GetInstance().GetAlgorithm()->StopSolving();
}
...
{
	ScheduleTest::GetInstance().GetAlgorithm()->StopSolving();
	ScheduleTest::GetInstance().GetObserver().WaitEvent();
